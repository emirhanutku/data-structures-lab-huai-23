...
public:
    ...
    void removeR(std::string &line);
    Block setBlock(const vector<vector<std::string>>& blocks);
    vector<vector<bool>> rotateRight(const vector<vector<bool>>& oldShape);
    vector<vector<bool>> rotateLeft(const vector<vector<bool>>& oldShape);
    void setRotation(Block &block);
